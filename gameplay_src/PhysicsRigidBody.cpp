        ...
        : PhysicsCollisionObject(node, group, mask), _body(NULL), _mass(parameters.mass), _constraints(NULL), _inDestructor(false)
{
    ...
    _collisionShape = Game::getInstance()->getPhysicsController()->createShape(node, shape, &centerOfMassOffset, false); //parameters.mass != 0.0f);
...
void PhysicsRigidBody::forceActivation(int state)
{
	_body->forceActivationState(state);
}
